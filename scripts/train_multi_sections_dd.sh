...
# train for some time on many differet sections
# allows agent to gain experience deeper in the game more quickly
# M1: 1M env steps in 12 min
# A10: 10M env steps in XX min

# ./train_multi_sections_dd.sh cpu v100
# ./train_multi_sections_dd.sh gpu v100_A10
...
case "$1" in
  cpu)
    ...
    for i in {1..10000}
    do
        for STAGE in 1-1-1 1-1-2 1-1-3 1-1-4 1-2-1
        do
            let "TRAIN_FOR_ENV_STEPS+=STEPS_DELTA"
            echo "Starting train loop $i on stage $STAGE"
            python -m sf_examples.retro.train_retro --algo=APPO --env=${ENV} --experiment="${ENV}_${EXPERIMENT_SUFFIX}" --state="1Player.ModeA.Level${STAGE}" \
              --device=cpu --with_wandb=True --wandb_project="${ENV}" --train_for_env_steps=${TRAIN_FOR_ENV_STEPS}
            echo "Finishing train loop $i on stage $STAGE"
            sleep 5  # let things settle before starting next run
            echo "Finishing sleep loop $i on stage $STAGE"
